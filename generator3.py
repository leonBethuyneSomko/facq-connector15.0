...
LISTED_FILES_EXTENSIONS = ['.xml', '.js', '.csv', '.po']


def should_list_file(filename):
    return any((filename.endswith(extension) for extension in LISTED_FILES_EXTENSIONS))
...
def list_sources(paths):
    ...
    try:
        for path in paths:
            ...
            for root, files in walk_python_path(path):
                for name in files:
                    if name.endswith('.py') or name.endswith('-nspkg.pth') or should_list_file(name):
